...
import filterLogic from '@salesforce/apex/Reim_FilterListLogic.filterLogic' //Need to check
//import getProjectLocationValues from '@salesforce/apex/reimGetPicklistTypeFields.getProjectLocationValues' //Need to check
//import getProjectTypeValues from '@salesforce/apex/reimGetPicklistTypeFields.getProjectTypeValues'         //Need to check
//import getProjectStatusValues from '@salesforce/apex/reimGetPicklistTypeFields.getProjectStatusValues'     //Need to check
import getProjects from '@salesforce/apex/S_InventoryDashboardController.getProjects'
import getTowers from '@salesforce/apex/S_InventoryDashboardController.getTowers'
...
export default class S_InventoryDashboard extends NavigationMixin(LightningElement) {
    columns=columns;
    ...
    @wire(getProjects)
    wiredProjectValues({error, data}) {
        if (data) {
            this.projectOptions = data.map((projectOptions) => {
                return {label: projectOptions, value: projectOptions};
            });
        } else if (error) {
            console.error(error);
        }
    ...
    @wire(getTowers, { projectName : '$selectedProject'})
    wiredTowerValues({error, data}) {
        if (data) {
            this.towerOptions = data.map((towerOptions) => {
                return {label: towerOptions, value: towerOptions};
            });
        } else if (error){
            console.error(error);
        }
